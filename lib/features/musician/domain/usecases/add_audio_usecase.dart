...
final addAudioUseCaseProvider = Provider<AddAudioUseCase>((ref) {
  final repository = ref.read(musicianRepositoryProvider);
  return AddAudioUseCase(repository: repository);
});

class AddAudioUseCase implements UsecaseWithParms<MusicianEntity, List<File>> {
  final IMusicianRepository _repository;

  AddAudioUseCase({required IMusicianRepository repository})
    : _repository = repository;

  @override
  Future<Either<Failure, MusicianEntity>> call(List<File> files) {
    return _repository.addAudio(files);
  }
}
