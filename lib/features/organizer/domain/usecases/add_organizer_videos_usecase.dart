...
final addOrganizerVideosUseCaseProvider =
    Provider<AddOrganizerVideosUseCase>((ref) {
  final repository = ref.read(organizerRepositoryProvider);
  return AddOrganizerVideosUseCase(repository: repository);
});

class AddOrganizerVideosUseCase
    implements UsecaseWithParms<OrganizerEntity, List<File>> {
  final IOrganizerRepository _repository;

  AddOrganizerVideosUseCase({required IOrganizerRepository repository})
