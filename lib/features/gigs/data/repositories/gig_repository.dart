...
class GigRepository implements IGigRepository {
  ...
  Future<Either<Failure, List<GigEntity>>> getAllGigs({
    ...
    try {
      ...
      return Left(
        ApiFailure(message: response.data['message'] ?? 'Failed to load gigs'),
      );
  ...
  @override
  Future<Either<Failure, void>> applyToGig(
    String gigId,
    String coverLetter,
  ) async {
    try {
      ...
      return Left(
        ApiFailure(
          message: response.data['message'] ?? 'Failed to apply to gig',
        ),
      );
    } on DioException catch (e) {
      return Left(
        ApiFailure.fromDioException(e, fallback: 'Failed to apply to gig'),
      );
    } catch (e) {
      return Left(ApiFailure(message: e.toString()));
    }
